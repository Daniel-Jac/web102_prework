...
const totalFundsRaised = GAMES_JSON.reduce((accumulated, game) => accumulated + game.pledged, 0);
const totalNumberOfGames = GAMES_JSON.length;


// create a string that explains the number of unfunded games using the ternary operator
const infoString = `A total of $${totalFundsRaised.toLocaleString()} has been raised for ${totalNumberOfGames} games. Currently, ${
    numUnfundedGames === 1
...
infoParagraph.innerHTML = infoString;
...
firstGameElement.textContent = `${mostFundedGame.name} - $${mostFundedGame.pledged.toLocaleString()}`;
firstGameContainer.appendChild(firstGameElement);

// do the same for the runner up item

const secondGameElement = document.createElement("p");
secondGameElement.textContent = `${secondMostFundedGame.name} - $${secondMostFundedGame.pledged.toLocaleString()}`;
